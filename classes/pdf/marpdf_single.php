...
$learnsign = $lib->learnsigned($userid, $courseid);
$ntasign = $lib->ntasign($userid, $courseid);
...
$pdf->Ln();
$pdf->setFont($font, 'B', $tablehead);
$pdf->MultiCell($table7width*4, $heighth+2, $strings['act_sum'], 1, 'C', 1, 0, '', '', true);
$pdf->Ln();
$pdf->setFont($font, '', $tabletext);
$pdf->MultiCell($table7width*4, $heightt, $array[19], 1, 'L', 0, 0, '', '', true);
$pdf->Ln();
$pdf->Cell(0,0,'');
...
$pdf->Ln();
$pdf->setFont($font, 'B', $tablehead);
$pdf->MultiCell($table10width, $heightt*2, $strings['employer_comment'] , 1, 'C', 1, 0, '', '', true);
$pdf->setFont($font, '', $tabletext);
$pdf->MultiCell($table10width*3, $heightt*2, "MonthlyActivityRecord-$username-$coursename-EmployerComment.pdf", 1, 'L', 0, 0, '', '', true);
$pdf->Ln();
$pdf->Cell(0,0,'');
...
if($array[25] !== null && !empty($array[25])){
    ...
    //Trying to solve the issue of signature not appearing on dev site
    //$img = '<img src="@'.(substr_replace($ntasign, '', 0, 21)).'">';
    //$pdf->writeHTML($img, true, false, false, true, false);
}
...
\local_offthejob\event\viewed_mar_pdf::create(array('context' => $context, 'relateduserid' => $userid, 'courseid' => $courseid, 'other' => $id))->trigger();